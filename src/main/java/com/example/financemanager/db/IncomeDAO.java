...
public class IncomeDAO {
    ...
    private static final String autoentrepriseColumn = "autoentreprise";
    private static final String passifIncomeColumn = "passifIncome";
        ...
        try (Connection connection = Database.connect()) {
            ...
            while (rs.next()) {
                incomes.add(new Income(
                        ...
                        rs.getFloat(incomeColumn),
                        rs.getFloat(helpColumn),
                        rs.getFloat(autoentrepriseColumn),
                        rs.getFloat(passifIncomeColumn),
    ...
    public static void insertIncome(Income income) {
        //update database
        String query = "INSERT INTO income(date, income, help, autoentreprise, passifIncome, other) VALUES (?, ?, ?, ?, ?, ?)";

        try (Connection connection = Database.connect()) {
            PreparedStatement statement = connection.prepareStatement(query);
            statement.setString(1, income.getDate().format(DATE_FORMAT));
            statement.setFloat(2, income.getIncome());
            statement.setFloat(3, income.getHelp());
            statement.setFloat(4, income.getAutoentreprise());
            statement.setFloat(5, income.getPassifIncome());
    ...
    public static List<Income> findLastIncomeEndingAtCurrentMonth(int numberOfLine, LocalDate currentMonth) {
        ...
        List<Income> lastIncome = new ArrayList<>();

        try (Connection connection = Database.connect()) {
            PreparedStatement statement = connection.prepareStatement(query);
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
                lastIncome.add(new Income(
                        LocalDate.parse(rs.getString(dateColumn), DATE_FORMAT),
                        rs.getFloat(incomeColumn),
                        rs.getFloat(helpColumn),
                        rs.getFloat(autoentrepriseColumn),
